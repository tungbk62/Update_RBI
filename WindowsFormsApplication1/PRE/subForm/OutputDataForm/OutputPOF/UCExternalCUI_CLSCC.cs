...
namespace RBI.PRE.subForm.OutputDataForm.OutputPOF
{
    public partial class UCExternalCUI_CLSCC : UserControl
    {
        ...
        public UCExternalCUI_CLSCC(int ID)
        {
            InitializeComponent();
            initinput(ID);
            //Calculate();
        ...
        private void initinput(int ID)
        {
            ...
            txtMaxTemper.Text = stream.MaxOperatingTemperature.ToString();
            txtMinTemper.Text = Convert.ToString(stream.MinOperatingTemperature);
            txtCoatingDate.Text = Convert.ToString(coat.ExternalCoatingDate);
            txtExternalEnvironment.Text = eq.ExternalEnvironment;
            txtCoatingDate.Text = Convert.ToString(coat.ExternalInsulation);
            txtChlorides.Text = Convert.ToString(coat.InsulationContainsChloride);
            txtInstalationCondition.Text = coat.InsulationCondition;
            txtExternalInstalationType.Text = coat.ExternalInsulationType;
            txtComplexityProtrusions.Text = component.ComplexityProtrusion;
        }
