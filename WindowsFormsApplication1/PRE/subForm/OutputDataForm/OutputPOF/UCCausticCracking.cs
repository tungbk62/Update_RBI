...
namespace RBI.PRE.subForm.OutputDataForm.OutputPOF
{
    public partial class UCCausticCracking : UserControl
    {
        ...
        public UCCausticCracking(int ID)
        {
            InitializeComponent();
            initinput(ID);
            //Calculate();
        ...
        private void initinput(int ID)
        {
            ...
            txtMaxTem.Text = Convert.ToString(stream.MaxOperatingTemperature);
            txtNaOH.Text = Convert.ToString(stream.NaOHConcentration);
            txtCrack.Text = Convert.ToString(false);
            txtStreamOut.Text = Convert.ToString(eq.SteamOutWaterFlush);
            txtPHWT.Text = eq.PWHT != 1 ? "False" : "True";
            txtHeatTraced.Text = Convert.ToString(eq.HeatTraced);
        }
