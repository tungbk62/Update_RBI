...
namespace RBI.PRE.subForm.OutputDataForm.OutputPOF
{
    public partial class UCInternalThinning : UserControl
    {
        ...
        public UCInternalThinning(int ID)
        {
            InitializeComponent();
            initinput(ID);
            //Calculate();
        ...
        private void initinput(int ID)
        {
            ...
            txtNorminal.Text = Convert.ToString(component.NominalThickness);
            txtLevelConfidence.Text = component.ConfidenceCorrosionRate;
            txtMinMeasured.Text = "E";
            txtMinMeasured1.Text = "E";
            txtMinRequired.Text = Convert.ToString(component.MinReqThickness);
            if (coat.InternalLining == 1)
                txtOnline.Text = "True";
            else txtOnline.Text = "False";
            txtCorrosionAllowance.Text = Convert.ToString(material.CorrosionAllowance);
            txtStructuralThickness.Text = Convert.ToString(component.StructuralThickness);
            txtMaintenance.Text = "E";
            txtAdjust.Text = eq.AdjustmentSettle;
            txtWelded.Text = Convert.ToString(eq.ComponentIsWelded);
            txtCurrent1.Text = Convert.ToString(component.CurrentCorrosionRate);
            txtCurrent1.Text = Convert.ToString(component.CurrentCorrosionRate);
            txtMinStructural.Text = Convert.ToString(component.MinimumStructuralThicknessGoverns);
            txtLastInspectionRate.Text = "E";
        }
